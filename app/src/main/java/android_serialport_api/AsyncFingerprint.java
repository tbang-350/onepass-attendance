...
public class AsyncFingerprint extends Handler {
	...
	/**
	 * ??ImageBuffer?е??????????????????????charBuffer1??CharBuffer2??
	 * 
	 * @param bufferId
	 *            ??CharBuffer1:1h,CharBuffer2:2h??
	 * @return ??????????? ?????=00H?????????????? ?????=01H???????д? ?????=06H??????????????????????
	 *         ?????=07H????????????????????????????????????? ?????=15H????????????????Ч?????????????? ?????=-1
	 *         ??????
	 */
	...
	/**
	 * ?????????????壬??CharBuffer1??CharBuffer2?е?????????????????壬???????CharBuffer1??CharBuffer2
	 * ?С?
	 * 
	 * @return ??????????? ?????=00H????????? ?????=01H???????д? ?????=0aH????????????????????????????
	 *         ?????=-1 ??????
	 */
	...
	/**
	 * ??CharBuffer?е???崢?浽?????pageId???flash?????λ?? bufferId:????1h??2h
	 * pageId????Χ?0~1010
	 * 
	 * @return ??????????? ?????=00H????????? ?????=01H???????д? ?????=0bH???PageID????????Χ
	 *         ?????=18H???дFLASH???? ?????=-1 ??????
	 */
	...
	/**
	 * ??flash ??????????pageId??????????????建????CharBuffer1??CharBuffer2
	 * bufferId:????1h??2h pageId????Χ?0~1023
	 * 
	 * @param index
	 *            pageId??
	 * @return ??????????? ?????=00H?????????? ?????=01H???????д? ?????=0cH????????д??????д?
	 *         ?????=0BH???PageID????????Χ ?????=-1 ??????
	 */
	...
	/**
	 * ??????CharBuffer1??CharBuffer2?е???????? ????:??λ??????????????滹??????÷?????÷???????50?????????
	 * 
	 * @return true?????????? false????????
	 */
	 ...
	 * ?????????????????????λ??
	 * 
	 * @return byte[]??length??С?36k ????????????????????????????4bits null:??????
	 */
	...
	private byte[] toBmpByte(int width, int height, byte[] data) {
		...
		try {
			...
			// ??????????????
			int bfType = 0x424d; // λ?????????0??1????
			int bfSize = 54 + 1024 + width * height;// bmp??????С??2??5????
			int bfReserved1 = 0;// λ????????????????0??6-7????
			int bfReserved2 = 0;// λ????????????????0??8-9????
			int bfOffBits = 54 + 1024;// ?????????λ???????????????????????10-13????

			// ??????????????????????????????????????????
			// ?????????????д??????????????????????????????
			// ????????????
			dos.writeShort(bfType); // ????λ????????'BM'
			dos.write(changeByte(bfSize), 0, 4); // ????λ??????С
			dos.write(changeByte(bfReserved1), 0, 2);// ????λ??????????
			dos.write(changeByte(bfReserved2), 0, 2);// ????λ??????????
			dos.write(changeByte(bfOffBits), 0, 4);// ????λ?????????

			// ??????????????
			int biSize = 40;// ????????????????14-17????
			int biWidth = width;// λ????18-21????
			int biHeight = -height;// λ?????22-25????
			int biPlanes = 1; // ????豸??????????1??26-27????
			int biBitcount = 8;// ????????????λ????28-29????????????1λ????????4λ??16?????8λ??256???????24λ????????????
			int biCompression = 0;// λ???????????????0???????????30-33??????1??BI_RLEB??????????2??BI_RLE4????????????
			int biSizeImage = width * height;// ???λ??????С???????????????????С??34-37????
			int biXPelsPerMeter = 0;// λ????????????????????38-41?????????????????
			int biYPelsPerMeter = 0;// λ?????????????????????42-45?????????????????
			int biClrUsed = 256;// λ??????????????е????????46-49??????????0????????????????
			int biClrImportant = 0;// λ????????????????????(50-53???)??????0??????????????
			...
			dos.write(changeByte(biWidth), 0, 4);// ????λ????
			dos.write(changeByte(biHeight), 0, 4);// ????λ????
			dos.write(changeByte(biPlanes), 0, 2);// ????λ???????豸????
			dos.write(changeByte(biBitcount), 0, 2);// ????????????????????
			dos.write(changeByte(biCompression), 0, 4);// ????λ??????????
			dos.write(changeByte(biSizeImage), 0, 4);// ????λ???????С
			dos.write(changeByte(biXPelsPerMeter), 0, 4);// ????λ??????????
			dos.write(changeByte(biYPelsPerMeter), 0, 4);// ????λ??????????
			dos.write(changeByte(biClrUsed), 0, 4);// ????λ????????????
			dos.write(changeByte(biClrImportant), 0, 4);// ????λ???ù???????????????
